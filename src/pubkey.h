...
public:
    ...
    /** Verify that this key is obtained from the x-only pubkey `naked` after applying:
     * - the tweak with `data` (unless it's null);
     * - the taptweak with `merkle_root` (unless it's null).
     */
    bool CheckDoubleTweak(const XOnlyPubKey& naked, const uint256* data, const uint256* merkle_root) const;
